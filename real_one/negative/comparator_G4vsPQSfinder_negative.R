setwd('/home/dimitri/rishav/table_comparator_G4hunt_pqsfinder/real_one/negative')
getwd()
...
tbl <- read_excel("hgg2_negative.xlsx", sheet = 1)
...
gr <- GRanges(seqnames = tbl2$chr,
              ...
              ranges   = IRanges(end = tbl2$start, start = tbl2$end),
              strand   = tbl2$strand,
              gene     = tbl2$gene)

# --------------------------------------------------------------------
# 3. Extract sequences from hg38k
seqs <- getSeq(hg38, gr)
...
pqs_bed_df <- do.call(bind_rows, lapply(seq_along(pqs_hits), function(i) {
  ...
  if (gene_strand == "-") {
    # For minus strand: the sequence was reverse complemented
    # So position 1 in the PQS corresponds to gene_end in genomic coords
...
write.table(bed_output, 
            file = "pqsfinder_all_genes_negative.bed",
            sep = "\t",
            quote = FALSE,
            row.names = FALSE,
            col.names = FALSE)

cat("Created: pqsfinder_all_genes_negative.bed\n")
cat("Total PQS motifs found:", nrow(bed_output), "\n")
